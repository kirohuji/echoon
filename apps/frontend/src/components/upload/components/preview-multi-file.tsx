    ...
    >
      ...
      {files.map((file) => {
        ...
        if (thumbnail) {
          return (
            <Box component="li" key={name} sx={{ display: 'inline-flex' }}>
              <FileThumbnail
                imgSx={undefined} tooltip
                imageView
                file={file}
                onDownload={() => onRemove?.(file)}
                sx={{
                  width: 80,
                  height: 80,
                  // border: (theme) =>
        ...
        return (
          ...
          >
            <FileThumbnail tooltip={false} imageView={false} onDownload={() => onRemove?.(file)} sx={undefined} imgSx={undefined} file={file} {...slotProps?.thumbnail} />
